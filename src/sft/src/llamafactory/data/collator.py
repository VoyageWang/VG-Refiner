...
# 1. ADD MASK TOOL FUCTION (Find the positions of <tool> and set the corresponding label_id = -100.)
# TODO `14172` is token id of `tool` in Qwen-2.5-VL . You can set your own task-oriented token id.
def mask_SpecialResp_labels(input_ids, labels, start_token_ids=[14172], end_token_ids=[14172]):
    """
    Args:
        input_ids: torch.Tensor of shape [batch_size, seq_len]
        labels: torch.Tensor of shape [batch_size, seq_len]
        start_token_ids: list of int, token ids representing the start sequence (appears once)
        end_token_ids: list of int, token ids representing the end sequence (appears once)
    
    Returns:
        masked_labels: torch.Tensor with specified ranges masked as -100
    """
    masked_labels = labels.clone()
    batch_size, seq_len = input_ids.size()
    start_len = len(start_token_ids)
    ...
    for i in range(batch_size):
        ...
        # Find start sequence (only first occurrence)
        for pos in range(seq_len - start_len + 1):
            if all(current_input[pos + k] == start_token_ids[k] for k in range(start_len)):
                start_pos = pos
                break

        # Find end sequence (only first occurrence after start_pos)
        if start_pos is not None:
            for pos in range(start_pos+1, seq_len - end_len + 1):
                if all(current_input[pos + k] == end_token_ids[k] for k in range(end_len)):
                    end_pos = pos + end_len - 1  # make inclusive
                    break

        # Mask the range if both markers found
        if start_pos is not None and end_pos is not None:
            if len(end_token_ids) == 1: end_pos += 1
            if len(start_token_ids) == 1: start_pos -= 1
            masked_labels[i, start_pos:end_pos + 1] = -100
            print('****** mask tool label here ... *******')
...
class SFTDataCollatorWith4DAttentionMask(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        if 'labels' in features.keys() and 'input_ids' in features.keys(): 
            try:
                # mask tool response here.
                label_with_tool_masked = mask_SpecialResp_labels(features['input_ids'], features['labels'] )
                features['labels'] = label_with_tool_masked
            except:
                pass
