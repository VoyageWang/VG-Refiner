...
class RayPPOTrainer:
    ...
    def _validate(self) -> Dict[str, Any]:
        ...
        for batch_dict in self.val_dataloader:
            ...
            if self.config.data.use_toolcall:                
                test_gen_batch = test_batch.pop(
                    batch_keys=["input_ids", "attention_mask", "position_ids"],
                    non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data","tool_response"],
                )
            else:
                test_gen_batch = test_batch.pop(
                    batch_keys=["input_ids", "attention_mask", "position_ids"],
        ...
        while True:
            ...
            if self.config.data.use_toolcall:
                if "multi_modal_data" in new_batch.non_tensor_batch.keys():
                    ...
                    raise NotImplementedError("Key ``tool_response`` in your dataset is necessary for vlm + toolcall :-( ")
            else:
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                with timer("gen", timing_raw):
                    # 这个是指示 Actor 工作节点准备好用于高效推理的生成引擎（可能是 vLLM）
                    # 因为需要算组间的,需要roll out
                    self.actor_rollout_ref_wg.prepare_rollout_engine()
