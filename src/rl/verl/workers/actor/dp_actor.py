...
class DataParallelPPOActor(BasePPOActor):
    ...
    # wrong Flash Attention not suport
    def _forward_micro_batch_withAttn2Img(self, micro_batch: Dict[str, torch.Tensor], temperature: float) -> torch.Tensor:
        """
        Returns:
            Attention : # (bs, response_len)
        """
        input_ids = micro_batch["input_ids"]
        batch_size, seqlen = input_ids.shape
        attention_mask = micro_batch["attention_mask"]
        ...
        # 如何不用 flash attention ? 如何从这一步开始，走类似 huggingface 的 forward ?
        return 
    ...
    def update_policy(self, data: DataProto) -> Dict[str, Any]:
        ...
        if "response_mask_tool" in data.batch.keys():
            select_keys.append("response_mask_tool")
        ...
        for _ in range(self.config.ppo_epochs):
            ...
            for mini_batch in mini_batches:
                ...
                for micro_batch in micro_batches:
                    ...
                    if 'response_mask_tool' in model_inputs:  # mask 掉 tool_response ```<tool>...</tool>```                  
                        response_mask = model_inputs['response_mask_tool']
