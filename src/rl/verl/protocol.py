...
def union_tensor_dict(tensor_dict1: TensorDict, tensor_dict2: TensorDict) -> TensorDict:
    ...
    # Unlock if locked, then update (compatible with different tensordict versions)
    is_locked = getattr(tensor_dict1, 'is_locked', False)
    if is_locked:
        tensor_dict1 = tensor_dict1.clone(recurse=False)
        tensor_dict1.update(tensor_dict2)
        # Try to lock if the method exists
        if hasattr(tensor_dict1, 'lock'):
            return tensor_dict1.lock()
        elif hasattr(tensor_dict1, 'lock_'):
            tensor_dict1.lock_()
            return tensor_dict1
        else:
            return tensor_dict1
