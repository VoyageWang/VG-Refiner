...
class RLHFDataset(Dataset):
    ...
    def _filter_overlong_prompts(self, example: Dict[str, Any]) -> bool:
        ...
        if self.image_key in example:
            ...
            if isinstance(images, dict):  # Single image as dict with 'bytes' key
                images = [images]  # Convert to list for uniform processing
            elif isinstance(images, list) and len(images) > 0 and self.image_dir is not None and isinstance(images[0], str):  # image paths
    ...
    def __getitem__(self, index):
        ...
        if self.image_key in example:
            ...
            model_inputs = self.processor(processed_images, [prompt], add_special_tokens=False, return_tensors="pt")
            # TODO 不会是 qwen 哥的 processor 重新给我缩放了吧，不会啊，在 min_pixel 与 max_pixel 之间
        ...
        # Handle answer field with fallback
        if self.answer_key in example:
            example["ground_truth"] = example.pop(self.answer_key)
        elif "ground_truth" in example:
            # Use existing ground_truth field if answer_key is not available
            example["ground_truth"] = example.pop("ground_truth")
        else:
            # Fallback to empty string if no answer field is found
